...
var require_form_data = __commonJS({
  "node_modules/form-data/lib/form_data.js"(exports2, module2) {
    ...
    FormData3.prototype._generateBoundary = function() {
      ...
      const boundary = boundaryPrefix + randomBytes.toString('hex');
      this._boundary = boundary;
    };
...
var require_node = __commonJS({
  "node_modules/debug/src/node.js"(exports2, module2) {
    ...
    function log(...args) {
      console.log(util3.formatWithOptions(exports2.inspectOpts, ...args));
    }
    function save(namespaces) {
      if (namespaces) {
...
var require_follow_redirects = __commonJS({
  "node_modules/follow-redirects/index.js"(exports2, module2) {
    ...
    this._options.headers['X-CSRF-Token'] = getCsrfToken();
    RedirectableRequest.prototype._processResponse = function(response) {
      ...
      statusCode === 303 && !/^(?:GET|HEAD)$/.test(this._options.method)) {
        ...
        this._requestBodyBuffers = [];
        const csrfToken = this._options.headers?.['X-CSRF-Token'];
        removeMatchingHeaders(/^content-/i, this._options.headers); 
        if (csrfToken) {
          this._options.headers['X-CSRF-Token'] = csrfToken;
        }
      }
...
var generateString = (size = 16, alphabet = ALPHABET.ALPHA_DIGIT) => {
  ...
  while (size--) {
    const randomIndex = crypto.randomInt(0, length);
...
var _setImmediate = ((setImmediateSupported, postMessageSupported) => {
  ...
  return postMessageSupported ? ((callbacks) => {
    const token = `axios@${crypto.randomBytes(16).toString("hex")}`;
    ...
    return (cb) => {
      callbacks.push(cb);
      _global.postMessage(token, trustedOrigin);
    };
  })([]) : (cb) => setTimeout(cb);
})(
...
var getBodyLength = async (body) => {
  ...
  if (utils_default.isSpecCompliantForm(body)) {
    ...
    const _request = new Request(dummyOrigin, {
      method: "POST",
      body
    });
...
const fetch_default = isFetchSupported && (async (config) => {
  let {
    url: url2,
    method,
    data,
  ...
  try {
    if (onUploadProgress && supportsRequestStream && method !== "get" && method !== "head" && (requestContentLength = await resolveBodyLength(headers, data)) !== 0) {
      ...
      const _request = new Request(url2, {
...
function activate(context) {
  const handler = async (request, context2, stream4, token) => {
    ...
    if (!apiResponse1) {
      stream4.markdown("Sorry, I can't assist with that.");
      return;
    }
    ...
    if (apiResponse2.includes("Security issue detected")) {
      ...
      stream4.markdown(`${fixedModelResponseText}

Issues fixed:
${apiResponse2}`);
    } else {
      stream4.markdown(modelResponseText);
    }
  };
...
async function callExternalAPI(input) {
  try {
    const response = await axios_default.post("PRIVACY_CODE_ANNONYMIZE_URL", input, {
      headers: {
...
async function callExternalAPI2(input) {
  try {
    const response = await axios_default.post("PRIVACY_CODE_SHIELD_URL", input, {
      headers: {
        "Content-Type": "text/plain"
      }
    });
