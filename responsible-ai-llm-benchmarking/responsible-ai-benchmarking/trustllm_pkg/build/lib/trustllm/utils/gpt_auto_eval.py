...
@retry(wait=wait_random_exponential(min=1, max=15), stop=stop_after_attempt(10))
def get_res(string, model='gpt-4-1106-preview', temp=0):
    ...
    if trustllm.config.azure_openai:
        ...
        openai.api_version = trustllm.config.azure_api_version
        try:
            ...
            if str(e).__contains__("Azure OpenAI's content management policy"):
                return "Azure OpenAI's content management policy"
