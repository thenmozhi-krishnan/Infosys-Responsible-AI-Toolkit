...
class LLMGeneration:
    ...
    def generation(self, model_name, prompt, tokenizer, model, temperature=None):
        ...
        try:
            #check if config.inhouse is true then call the gen_inhouse
            if (self.online_model and trustllm.config.inhouse_url):
                ans=gen_inhouse(prompt,temperature)
            elif (model_name in self.online_model_dict) and ((self.online_model and self.use_replicate) or (self.online_model and self.use_deepinfra)):
    ...
    def process_element(self, el, model, model_name, tokenizer, index, temperature, key_name='prompt'):
        ...
        try:
            ...
            log.error(f"Error processing element at index {index}: {e}")
    ...
    def process_file(self, data_path, save_path, model_name, tokenizer, model, file_config, key_name='prompt'):
        ...
        if os.path.basename(data_path) not in file_config:
            log.debug(f"{os.path.basename(data_path)} not in file_config")
            return
    ...
    def run_single_test(self):
        ...
        if test_func:
            ...
            log.debug("Invalid test_type. Please provide a valid test_type.")
            return None
    ...
    def generation_results(self, max_retries=10, retry_interval=3):
        ...
        if not os.path.exists(self.data_path):
            log.debug(f"Dataset path {self.data_path} does not exist.")
            return None
        ...
        for attempt in range(max_retries):
            try:
                ...
                log.error(f"Test function failed on attempt {attempt + 1}: {e}")
                log.debug("Retrying test function...")
                time.sleep(retry_interval)

        log.debug("Test failed after maximum retries.")
        return None
