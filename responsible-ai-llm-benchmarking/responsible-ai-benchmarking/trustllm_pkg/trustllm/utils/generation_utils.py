...
def get_res_chatgpt(string, model, temperature):
    ...
    if not completion.choices[0].message['content']:
        raise ValueError(
            "The response from the API is NULL or an empty string!")
...
@retry(wait=wait_random_exponential(min=1, max=3), stop=stop_after_attempt(3))
def gen_inhouse(prompt, temperature):
    #using request module
    try:
        result = http_requ(prompt,temperature)
    
    #using gradio client
    # client = Client(src=trustllm.config.inhouse_url+"run", ssl_verify=False)
    # result = client.predict(
    #     prompt,  # str  in 'Input' Textbox component
    #     # int | float (numeric value between 0 and 100) in 'Max new tokens' Slider component
    #     100,
    #     fn_index=0,
    #     api_name="/predict"
    # )
