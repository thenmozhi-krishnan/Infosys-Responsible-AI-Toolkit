...
class BaichuanModel(LMMBaseModel):
    ...
    predict(input_text, **kwargs)
        Generates a prediction based on the input text.
    """
    def __init__(self, model_name,modelPath,max_new_tokens, temperature, device, dtype):
        super(BaichuanModel, self).__init__(model_name, max_new_tokens, temperature, device)
        from transformers import AutoTokenizer, AutoModelForCausalLM

        self.tokenizer = AutoTokenizer.from_pretrained(modelPath, torch_dtype=dtype, device_map=device, use_fast=False, trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained(modelPath, torch_dtype=dtype, device_map=device, trust_remote_code=True)
...
class YiModel(LMMBaseModel):
    ...
    def __init__(self, model_name,modelPath,max_new_tokens, temperature, device, dtype):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(modelPath, torch_dtype=dtype, device_map=device)
        self.model = AutoModelForCausalLM.from_pretrained(modelPath, torch_dtype=dtype, device_map=device)        
...
class MistralModel(LMMBaseModel):
    ...
    def __init__(self, model_name,modelPath,max_new_tokens, temperature, device, dtype):
        ...
        # self.tokenizer = AutoTokenizer.from_pretrained(self.model_name, torch_dtype=dtype, device_map=device)
        # self.model = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=dtype, device_map=device)
        # self.tokenizer.save_pretrained(r"D:\model\mistral")
        # self.model.save_pretrained(r"D:\model\mistral")
...
class PhiModel(LMMBaseModel):
    ...
    def __init__(self, model_name,modelPath,max_new_tokens, temperature, device, dtype):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(modelPath, trust_remote_code=True, torch_dtype=dtype, device_map=device)
        self.model = AutoModelForCausalLM.from_pretrained(modelPath, trust_remote_code=True, torch_dtype=dtype, device_map=device)
        ...
        # modelPath=os.path.join(os.getcwd(),f"downloadedModels\{self.model_name}")
        # print(modelPath)
        # print(1)
        # self.tokenizer.save_pretrained(os.path.join(modelPath,"tokenizer"))
        # print(2)
        # self.model.save_pretrained(os.path.join(modelPath,"model"))
        # print(3)
...
class T5Model(LMMBaseModel):
    ...
    def __init__(self,model_name,modelPath, max_new_tokens, temperature, device, dtype):
        ...
        #     print("In if path exists")
        #     self.tokenizer=T5Tokenizer.from_pretrained(os.path.join(modelPath,"tokenizer"), torch_dtype=dtype, device_map=device)
        #     self.model=T5ForConditionalGeneration.from_pretrained(os.path.join(modelPath,"model"), torch_dtype=dtype, device_map=device)
        ...
        #     self.tokenizer.save_pretrained(os.path.join(modelPath,"tokenizer"))
        #     self.model.save_pretrained(os.path.join(modelPath,"model"))
        # savedModelPath=os.path.join(modelPath,"model")
        # print(4)
        # print(savedModelPath)
        # savedTokennizerPath=os.path.join(modelPath,"tokenizer")
        # print(savedTokennizerPath)
        self.tokenizer = T5Tokenizer.from_pretrained(modelPath, torch_dtype=dtype, device_map=device)
        self.model = T5ForConditionalGeneration.from_pretrained(modelPath, torch_dtype=dtype, device_map=device)
