...
class LLMModel(object):
    ...
    def __init__(self,modelPath,model, max_new_tokens=20, temperature=0, device="cuda", dtype="auto", model_dir=None, system_prompt=None, api_key=None):
        print(1)
        self.model_name = model
        self.model = self._create_model(max_new_tokens,modelPath, temperature, device, dtype, model_dir, system_prompt, api_key)

    def _create_model(self, max_new_tokens,modelPath, temperature, device, dtype, model_dir, system_prompt, api_key):
        ...
        if model_class:
            if model_class == LlamaModel or model_class == VicunaModel:
                ...
                print(2)
                return model_class(self.model_name,modelPath, max_new_tokens, temperature, device, dtype)
        else:
